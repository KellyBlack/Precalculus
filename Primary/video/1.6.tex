...
\fbox{
  ...
  {
    \underline{Vertical shifts.} Assume that $c$ is a positive number.

    \noindent
    The graph of $y = f(x)+c$ is obtained from the graph of $y=f(x)$ by
    shifting it $c$ units upward.

    \noindent
    The graph of $y = f(x)-c$ is obtained from the graph of $y=f(x)$
    by shifting it $c$ units downward.
...
\item For this problem, let $f(x)=x^2$. Sketch the graph of the
  function $y=f(x) - 3$. Compare the domains and ranges of $y=f(x)$
  and $y=f(x)-3$.
...
\fbox{
  \parbox{\dimexpr\linewidth}%
  {
    ...
    \underline{Horizontal shifts.} Assume that $c$ is a positive
    number.

    \noindent
    The graph of $y = f(x-c)$ is obtained from the graph of $y=f(x)$
    by shifting it $c$ units to the right.

    \noindent
    The graph of $y = f(x+c)$ is obtained from the graph of $y=f(x)$
    by shifting it $c$ units to the left.
...
\fbox{
  \parbox{\dimexpr\linewidth}%
  {
    \underline{Reflection through the $x$-axis.} The graph of
...
\fbox{
  \parbox{\dimexpr\linewidth}%
  {
    ...
    stretching the graph of $y=f(x)$ vertically by a factor of $c$.
    If $c$ is between $0$ and $1$, the graph of $y=cf(x)$ is obtained
    by compressing the graph vertically by a factor of $1/c$.
  }
}
...
\fbox{
  \parbox{\dimexpr\linewidth}%
  {
    ...
    $y=f(-x)$ is obtained by reflecting the graph of $y=f(x)$ through
    the $y$-axis.
  }
}

%\begin{enumerate}
\item For the graph of $y=f(x)$ shown below, sketch the graph of
  $y=f(-x)$.
  ...
  \noindent
      \begin{tikzpicture}[y=1cm, x=1cm,font=\sffamily]
        \begin{scope} %[shift={(0,8)}]
          %% ticks
          \draw[xstep = 1, ystep=1.0,black,dashed,thick] % very thin,opacity=0.85,
                 (-5.0,-1.0) grid ( 5.0, 5.0);
             %% axis
           \draw[ultra thick,->] (-5.5,0) -- coordinate (x axis mid) (5.5,0)
                node[anchor = north west] {$x$}; 
           \draw[ultra thick,->] (0,-1.5) -- coordinate (y axis mid) (0,5.5) 
                node[anchor = east,shift={(-0.2,-0.2)}]  {$y$};

           \foreach \y in {-1,1,2,3,4,5} {
              \draw (1pt, \y) -- (-1pt, \y) node[yshift=-6,xshift=1,anchor=east] {$\y$};
            }
           \foreach \x in {-5,-4,...,-1,1,2,3,4,5} {
              \draw (\x,1pt) -- (\x,-1pt) node[yshift=-5,xshift=-1,anchor=north] {$\x$};
            }
...
\fbox{
  \parbox{\dimexpr\linewidth}%
  {
    \underline{Horizontal Compression/Stretching} Assume that $c$ is a
    positive number.  If $c>1$, the graph of $y=f(cx)$ is obtained by
    compressing the graph horizontally by a factor $c$. If $c$ is
    between $0$ and $1$, the graph of $y=f(cx)$ is obtained by
    stretching the graph of $y=f(x)$ horizontally by a factor of
    $1/c$.
  }
}
  ...
  \noindent
      \begin{tikzpicture}[y=0.7cm, x=0.7cm,font=\sffamily]
        \begin{scope} %[shift={(0,8)}]
          %% ticks
          \draw[xstep = 1, ystep=1.0,black,dashed,thick] % very thin,opacity=0.85,
                 (-8.0,-3.0) grid ( 8.0, 6.0);
             %% axis
           \draw[ultra thick,->] (-8.5,0) -- coordinate (x axis mid) (8.5,0)
                node[anchor = north west] {$x$}; 
           \draw[ultra thick,->] (0,-3.5) -- coordinate (y axis mid) (0,6.5) 
                node[anchor = east,shift={(-0.2,-0.2)}]  {$y$};
           ...
           \foreach \x in {-8,-6,...,-2,2,4,...,8} {
            ...
            \draw[ultra thick] (-6,6) -- (-2,1) -- (1,4);
            \fill[black] (-6,6) circle [radius=0.5ex];
            \fill[black] ( 1,4) circle [radius=0.5ex];
          \end{scope}

        \end{tikzpicture}


\item For the graph of $y=f(x)$ shown below, sketch the graph of $y =
  |f(x)|$.
  
  \begin{tikzpicture}[y=0.7cm, x=1.0cm,font=\sffamily]
    %% ticks
    \draw[step = 1, gray,dashed] (-4,-3) grid (6,3);
    %% axis
    \draw[thick,->] (-4.5,0) -- coordinate (x axis mid) (6.5,0) node[anchor = north west] {$x$};
    \draw[thick,->] (0,-3.5) -- coordinate (y axis mid) (0,3.5) node[anchor = north east] {$y$};
    ...
    \begin{scope}
      ...
      \draw[scale=1.0,domain=-4:6,smooth,variable=\x,very thick,black,samples=120] 
            plot ({\x},{2*sin(deg(pi*\x))}) node[anchor=south west] {$f(x)$};
      \fill[black] (-4,0) circle [radius=0.5ex];
      \fill[black] ( 6,0) circle [radius=0.5ex];
    \end{scope}
