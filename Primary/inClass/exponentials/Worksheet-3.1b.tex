...
\item Determine whether or not each relationship below is a function.
  ...
  so that it is one-to-one on the restricted domain so that the range
  of the function on the restricted domain is the same as the range of
...
\item The graph of a function, $h$, is given in the plot below, and a
  ...
  \item  Determine the value of $g(h^{-1}(2))$.
