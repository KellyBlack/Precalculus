...
\fbox{
  ...
  {    
    \noindent \underline{Horizontal Line Test.} A function is
    one-to-one if every horizontal line intersects the graph of $f$ at
    most once. Otherwise the function is not one-to-one.
  }
}

\begin{enumerate}
\item Determine whether or not the graph is the graph of a one-to-one
  function.
