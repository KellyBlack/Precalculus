...
\item A car moves along a straight track. The graph of the car's
  ...
  \begin{tikzpicture}[y=1.0cm, x=1.0cm,font=\sffamily]
    ...
    \draw[step = 1, gray, very thin,dashed,opacity=0.85] (\lowX, \lowY) grid ( \highX,\highY);
      % axis
    \draw[thick,->] (\lowX,0) -- coordinate (x axis mid) (\highX,0) node[anchor = north west] {time (minutes)};
    \draw[thick,->] (0,\lowY) -- coordinate (y axis mid) (0,\highY) node[anchor = south east] {Position (m)};
    \foreach \y in {1,2,...,\highY} {
      \draw (1pt, \y) -- (-1pt, \y) node[yshift=-6,xshift=-1,anchor=east] {$\y$};
    }
    \foreach \x in {1,2,...,\highX} {
      \draw (\x,1pt) -- (\x,-1pt) node[yshift=-5,xshift=-1,anchor=east] {$\x$};
    }
    \draw [black, very thick] plot coordinates {
      (0,0) (2,4) (4,4) (6,0)};
    \fill (0, 0) circle [radius=0.5ex];
    \fill (6, 0) circle [radius=0.5ex];
