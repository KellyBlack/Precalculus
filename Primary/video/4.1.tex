...
\fbox{
  ...
  {
    \begin{itemize}
    \item An \textbf{angle} is formed by rotating a ray around its
      endpoint.
    \item The original position of the ray is called the
      \textbf{initial side} of the angle.
    \item The ending position of the ray is called the
      \textbf{terminal side}.
    \item The endpoint of the ray is called the \textbf{vertex}.
    \end{itemize}
  }
}
...
\fbox{
  ...
  {
    \begin{itemize}
    \item An angle is said to be in \textbf{standard position} if its
      vertex is at the origin and its initial side is on the positive
      $x$-axis.
    \item  An angle is \textbf{positive} if it is rotated
      counterclockwise and \textbf{negative} if it is rotated
      clockwise.
    \item  Greek letters are commonly used to name the angles:
      $\alpha$ (alpha), $\beta$ (beta), $\theta$ (theta), $\gamma$
      (gamma), $\phi$ (phi). 
    \end{itemize}
  }
}
...
\begin{enumerate}
\item Draw a positive angle $\theta$ in standard position with its
  terminal side in Quadrant 3 and a negative angle $\beta$ with its
  terminal side in Quadrant 4. (Draw and label your axes.)
  \vfill

  \textbf{Degree Measure} We can measure angles using degrees noted by
  $^\circ$.  Certain angles have special names:
...
\item Convert $90^\circ, 150^\circ, 1^\circ, -135^\circ$ to radians.
  \vfill

\item Convert $\frac{5\pi}{6}, \frac{-5\pi}{6}, 3\pi, 1$ radian to
  degrees.
  \vfill
...
\item Find two positive angles and two negative angles coterminal to
  $\theta = 60^\circ$.
...
\fbox{
  \parbox{\dimexpr\linewidth}%
  {
    If $\theta$ is the radian measure of a central angle of a circle
    of radius $r$, and if $A$ is the area of the circular sector
    determined by $\theta$, then $A = \frac{1}{2}r^2 \theta$.
  }
}

\item Determine the area of the circular sector with central angle
  $\theta = 120^\circ$ when the radius of the circle is 5.
  \vfill
