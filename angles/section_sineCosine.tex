...
\begin{problem}
  ...
  \item Estimate the turtle's position graphically after it moves a
    ...
    \sideNote{Make a sketch of the unit circle first and estimate its
      location.}
...
\item A turtle moves around the edge of a circle of radius 1 meter.
  \begin{subproblem}
    ...
    \item As the turtle moves from a distance of 0 meters to $\frac{\pi}{2}$ meters
      ...
      its $y$ coordinate?
      \sideNote{Is it increasing or decreasing?}
    \vfill
...
\item \label{question:sunMovement} When viewed above the north pole of
  the Sun, the earth appears to move around the sun in a
  counter-clockwise direction. The path can be roughly approximated as
  a circle. It takes one year to make one revolution, and assume that
  the distance from the center of the sun to the earth is one solar
  unit. (See the image in the preclass activity.)
  \begin{subproblem}
...
\item \label{question:plainfield500} At the annual Plainfield $500\pi$
  race a tractor will make 250 laps around a circular track. The track
  has a radius of 1 km. A single tractor will make a trial run by
  going around the track at 1 km per hour. (It is not a very fast
  race.) The tractor starts on the point furthest East and is
  initially moving to the North.
