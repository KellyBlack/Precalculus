...
\begin{tikzpicture}[y=0.8cm, x=0.8cm,font=\sffamily]
  ...
  \draw[black,dashed,fill=black] (0,0) -- (150:3) circle (0.05) node[anchor=east] {$(x,1.5)$};
...
\item Determine the tangent of the angle $\psi$.
  (Your answer should be a number and not have an $y$ in it.)\\
...
\item Given the following, find the exact value of $\sin{(\alpha)}.$\\
  ...
  \text{and}\quad \tan{(\alpha)} \quad \text{is negative}$$\vfill

\newpage
\item A ship is anchored a distance of $d=150$ from a beach.  A
