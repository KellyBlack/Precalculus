...
\begin{tabular}{| p{0.8\textwidth} |} \hline
  \underline{Horizontal Compression/Stretching} Assume that $c$ is a positive number. 
  If $c>1$, the graph of $y=f(cx)$ is obtained by compressing the
  graph horizontally by a factor $c$. If $c$ is between $0$ and $1$,
  the graph of $y=f(cx)$ is obtained by stretching the graph of
  $y=f(x)$ horizontally by a factor of $1/c$.
 \\ \hline
\end{tabular} 
