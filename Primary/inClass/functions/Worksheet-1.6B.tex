...
\item The graphs of two functions, $u$ and $v$, are given in
  the plot below.

  \begin{tikzpicture}[y=0.75cm, x=0.75cm,font=\sffamily]
    %% ticks
    \draw[step = 1, gray,dashed] (-6,-3) grid (6,6);
    %% axis
    \draw[thick,->] (-6.5,0) -- coordinate (x axis mid) (6.5,0) node[anchor = north west] {$x$};
    \draw[thick,->] (0,-2.5) -- coordinate (y axis mid) (0,6.5) node[anchor = north east] {$y$};
    \foreach \y in {-3,-2,...,-1,1,2,...,6} {
    ...
    \begin{scope}
      ...
      \draw[scale=1.0,domain=0:4,smooth,variable=\x,very thick,black,samples=120] 
           plot ({\x/2-5},{3*sin(deg(pi*\x))+1}) node[anchor=south] {\huge $v$};
      \fill[black] (-5, 1) circle [radius=0.5ex];
      \fill[black] (-3, 1) circle [radius=0.5ex];
      \draw[scale=1.0,domain=0:4,smooth,variable=\x,very thick,black,samples=120] 
           plot ({\x},{sin(deg(pi*\x))}) node[anchor=south] {\huge $u$};
  ...
  \begin{enumerate}
  \item The function $v$ can be written in terms of $u$
    as $v(x)=A\cdot u(c\cdot x +d)+B$. Determine the values of $A$, $B$,
    $c$, and $d$.
    \vfill
  \item make a rough sketch of the function
    $w(x)=2u\left(x+4\right)+3$.
  \end{enumerate}
...
\begin{enumerate}
\item Write an equation for the function $g(x)$ that is obtained from
  $f(x)$ by first shifting left by 6, then compressing horizontally by
  a factor of 3.
\item Write an equation for the function $h(x)$ that is obtained from
  $f(x)$ by first compressing horizontally by a factor of 3, then
  shifting left by 2.
\item Compare your answers to part (a) and (b).  What does this tell
  you about the order in which we perform our transformations?
\end{enumerate}
...
\item The graphs of two functions, $u$ and $v$, are given in
  ...
  \begin{tikzpicture}[y=1.0cm, x=1.0cm,font=\sffamily]
    ...
    \begin{scope}
      %\clip(-4,-1) rectangle (8,5);
      \draw[scale=1.0,domain=0:4,smooth,variable=\x,very thick,black,samples=120,dashed] 
           plot ({\x*2-4},{3*sin(deg(pi*\x))+3}) node[anchor=south] {\huge $v$};
      \fill[black] (-4, 3) circle [radius=0.5ex];
      \fill[black] ( 4, 3) circle [radius=0.5ex];
      \draw[scale=1.0,domain=0:4,smooth,variable=\x,very thick,black,samples=120] 
           plot ({\x},{sin(deg(pi*\x))}) node[anchor=south] {\huge $u$};
  ...
  \item The function $v$ can be written in terms of $u$
    ...
    $c$, and $d$.
  \item make a rough sketch of the function
    $w(x)=u\left(\frac{1}{3}x+2\right)-4$.
  \end{enumerate}

\item The graphs of three functions, $f$, $g$, and $h$, are given in
  the plot below.
  ...
  \begin{tikzpicture}[y=1.0cm, x=1.0cm,font=\sffamily]
    ...
    \begin{scope}
      %\clip(-4,-1) rectangle (8,5);
      \draw[scale=1.0,domain=-2:3,smooth,variable=\x,very thick,black,samples=120] 
           plot ({(\x-4)},{cos(deg(3*pi/5*(\x+0.4)))+1}) node[anchor=south] {\huge $g$};
      \fill[black] (-6, 0) circle [radius=0.5ex];
      \fill[black] (-1, 2) circle [radius=0.5ex];
      \draw[scale=1.0,domain=-2:3,smooth,variable=\x,very thick,black,samples=120] 
           plot ({(\x+2)+1},{4-2*cos(deg(3/5*pi*(\x+0.4)))}) node[anchor=west] {\huge $h$};
      \fill[black] (1,6) circle [radius=0.5ex];
      \fill[black] (6,2) circle [radius=0.5ex];
      \draw[scale=1.0,domain=-2:3,smooth,variable=\x,very thick,black,samples=120] 
           plot ({\x+2},{cos(deg(3*pi/5*(\x+0.4)))}) node[anchor=west] {\huge $f$};
      \fill[black] (0,-1) circle [radius=0.5ex];
      \fill[black] (5, 1) circle [radius=0.5ex];
      %%\draw[scale=1.0,domain=-2:3,smooth,variable=\x,very thick,black,samples=120] 
      %%     plot ({\x-3},{cos(deg(3*pi/5*(\x+0.4)))-1}) node[anchor=south east] {\huge $g$};
